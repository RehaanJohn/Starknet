...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!user || amount === undefined) {
      return NextResponse.json({ error: 'User address and amount required' }, { status: 400 });
    }
    ...
    const result = await contract.set_chipi_balance(user, amountLow.toString(), amountHigh.toString());
